...
function generarTarlov(h){
  ...
  if(multi){
    return {
      descripcion: limpiar(`Se identifican quistes perineurales o de Tarlov en topografía sacra ${loc ? "a nivel de "+loc : ""}${lado}, hiperintensos en T2 e hipointensos en T1, de bordes bien definidos${med}.`),
      diagnostico: limpiar(`Quistes perineurales o de Tarlov${loc ? " a nivel de "+loc : ""}.`)
    };
  }
  return {
    descripcion: limpiar(`Se identifica quiste perineural o de Tarlov en topografía sacra ${loc ? "a nivel de "+loc : ""}${lado}, hiperintenso en T2 e hipointenso en T1, de bordes bien definidos${med}.`),
    diagnostico: limpiar(`Quiste perineural o de Tarlov${loc ? " a nivel de "+loc : ""}.`)
  };
}
